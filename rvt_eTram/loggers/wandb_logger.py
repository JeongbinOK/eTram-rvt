...
class WandbLogger(Logger):
    ...
    def _get_public_run(self):
        """
        Return a public run object or None.

        * wandb>=0.16: Use `.entity .project .id`
        * wandb offline/disabled: returns None so caller can skip.
        """
        if self._public_run is not None:
            return self._public_run

        experiment = self.experiment
        # RunDisabled: WANDB_MODE=disabled / dryrun
        if isinstance(experiment, RunDisabled):
            return None

        # Try public attrs first
        entity = getattr(experiment, "entity", None) or getattr(
            experiment, "_entity", None
        )
        project = getattr(experiment, "project", None) or getattr(
            experiment, "_project", None
        )
        run_id = getattr(experiment, "id", None) or getattr(experiment, "_run_id", None)

        if entity and project and run_id:
            try:
                self._public_run = wandb.Api().run(f"{entity}/{project}/{run_id}")
            except wandb.errors.CommError:
                # offline or no internet
                self._public_run = None
        else:
            self._public_run = None
    ...
    def _scan_and_log_checkpoints(
        ...
        # ─── NEW EARLY EXIT ────────────────────────────────
        if isinstance(self.experiment, RunDisabled):
            # wandb가 disabled/offline이면 아티팩트 처리 자체를 건너뜀
            return
        # ───────────────────────────────────────────────────
    ...
    def _rm_but_top_k(self, top_k: int):
        ...
        # ──── NEW GUARD ──────────────────────────────────────────
        if public_run is None:
            # wandb disabled/offline → nothing to prune, just return
            return
        # ─────────────────────────────────────────────────────────
