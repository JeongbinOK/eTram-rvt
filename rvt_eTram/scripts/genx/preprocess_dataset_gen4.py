...
#
# --------------------------------------------------------------------------- #
# Local replacement for utils.preprocessing._blosc_opts
# --------------------------------------------------------------------------- #
def _blosc_opts(complevel: int = 1, complib: str = "blosc:zstd", shuffle: str = "byte"):
    """
    Return HDF5 filter options for Blosc compression.
    ...
    complevel : int, 0‑9
        Compression level.
    complib : str
        Blosc compressor name, e.g. 'blosc:zstd', 'blosc:lz4'.
    shuffle : str
        'byte', 'bit', or 'none'.  Controls Blosc shuffle filter.
    """
    ...
    # If using blosc shuffle, disable h5py's native shuffle
    if shuffle_code > 0:
        args["shuffle"] = False
    return args
...
dataset_2_height = {"gen1": 240, "gen4": 720, "testdata": 720, "eightclass": 720}
dataset_2_width = {"gen1": 304, "gen4": 1280, "testdata": 1280, "eightclass": 1280}
...
dirs_to_ignore = {
    ...
    "testdata": (),
    "eightclass": (
        "train_night_0021",  # skip associated *_bbox.npy & *_td.h5
        "train_day_0024",  # skip this sequence as well
    ),
}
...
class H5Reader:
    def __init__(self, h5_file: Path, dataset: str = "gen4"):
        ...
        # TODO: update dataset name
        assert dataset in {"gen1", "gen4", "testdata", "eightclass"}
...
def prophesee_bbox_filter(labels: np.ndarray, dataset_type: str) -> np.ndarray:
    # TODO: update dataset name
    assert dataset_type in {"gen1", "gen4", "testdata", "eightclass"}
    ...
    min_box_diag = 60 if dataset_type in {"gen4", "testdata"} else 30
    # Corrected values from supplementary mat from paper for min_box_side!
    min_box_side = 20 if dataset_type in {"gen4", "testdata"} else 10
...
def get_base_delta_ts_for_labels_us(
    ...
    # TODO: update dataset name
    assert dataset_type in ["gen4", "testdata", "eightclass"]
...
def write_event_representations(
    ...
    with H5Reader(in_h5_file, dataset=dataset) as h5_reader, H5Writer(
        ...
        for idx_start, idx_end in tqdm(zip(start_indices, end_indices)):
...
if __name__ == "__main__":
    ...
    # TODO: update dataset name
    assert dataset in ("gen1", "gen4", "testdata", "eightclass")
    downsample_by_2 = True if dataset in ["gen4", "testdata", "eightclass"] else False
    ...
    for split in [train_path, val_path, test_path]:
        ...
        for npy_file in split.iterdir():
            ...
            # Skip sequences explicitly listed for this dataset, if any
            if dir_name in dirs_to_ignore.get(dataset, ()):
                continue
