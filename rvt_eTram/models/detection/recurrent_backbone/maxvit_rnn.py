...
class RNNDetector(BaseDetector):
    def __init__(self, mdl_config: DictConfig):
        ...
        use_plain_lstm = mdl_config.get('use_plain_lstm', False)  # RVT paper's Plain LSTM
        ...
        for stage_idx, (num_blocks, T_max_chrono_init_stage) in \
            ...
            stage = RNNDetectorStage(dim_in=input_dim,
                                     ...
                                     stage_idx=stage_idx,
                                     use_enhanced_convlstm=use_enhanced_convlstm,
                                     use_plain_lstm=use_plain_lstm)
...
class RNNDetectorStage(nn.Module):
    ...
    def __init__(self,
                 ...
                 use_enhanced_convlstm: bool = False,
                 use_plain_lstm: bool = False):
        super().__init__()
        ...
        # LSTM Selection Logic based on configuration
        # Priority: Plain LSTM (RVT paper) > Enhanced ConvLSTM > Standard ConvLSTM
        if use_plain_lstm:
            # RVT paper's Plain LSTM - proven 1.1% mAP improvement over ConvLSTM
            self.lstm = PlainLSTM2d(dim=stage_dim,
                                   cell_update_dropout=lstm_cfg.get('drop_cell_update', 0))
        elif use_enhanced_convlstm and stage_idx == 1:  # P2 stage (stride 8) only
            # Enhanced ConvLSTM for P2 stage when specifically requested
            self.lstm = LightweightEnhancedConvLSTM(
                ...
                cell_update_dropout=lstm_cfg.get('drop_cell_update', 0),
                enhancement_ratio=lstm_cfg.get('enhancement_ratio', 0.1),
                small_object_threshold=lstm_cfg.get('small_object_threshold', 0.3)
            )
        else:
            # Default: Standard DWSConvLSTM2d (current baseline)
